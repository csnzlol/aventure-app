...
export default function Squats() {
  ...
  useEffect(() => {
    const loadSetsData = async () => {
      const storedSets = await AsyncStorage.getItem('squats_sets');
      if (storedSets) {
  ...
  const handleCompleteSet = async () => {
    if (repsCompleted >= randomReps) {
      ...
      await AsyncStorage.setItem('squats_sets', newSetsCompleted.toString());
    }
  };
  ...
  const handleReset = () => {
    ...
    AsyncStorage.removeItem('squats_sets');
  };
